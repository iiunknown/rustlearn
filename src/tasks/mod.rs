...
impl fmt::Display for Task {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let created_at = self.created_at.with_timezone(&Local).format("%F %H:%h");
...
pub fn add_task(journal_file: PathBuf, task: Task) -> Result<()> {
    ...
    // Consume the file's content as a vertor of tasks.
...
pub fn complete_task(journal_file: PathBuf, task_position: usize) -> Result<()> {
    ...
    // Consume the file contents as a vector of tasks.
    let mut tasks = collect_tasks(&file)?;
    ...
    // Write the modified the task list back into the file.
    serde_json::to_writer(file, &tasks)?;
    Ok(())
}
